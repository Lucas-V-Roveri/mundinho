...
class LocalPreviewStore implements DataStore {
  ...
  async setCompleted({ itemId, completed, actor, section, entryKey }: Parameters<DataStore["setCompleted"]>[0]) {
    ...
    const row: ItemState = { world_id: this.worldId, item_id: itemId, section, entry_key: entryKey, completed, completed_by: completed ? actor : null, completed_at: completed ? nowIso() : null, updated_at: nowIso(), deleted: false, deleted_by: null, deleted_at: null };
    states[itemId] = row; localStorage.setItem(this.statesKey, JSON.stringify(states)); this.emit(); return row;
  }
  async addCustomItem(input: Parameters<DataStore["addCustomItem"]>[0], actor: Actor) {
    const custom = this.custom();
    const row: CustomItem = { ...input, id: crypto.randomUUID(), world_id: this.worldId, created_by: actor, created_at: nowIso(), deleted: false, deleted_by: null, deleted_at: null, updated_at: nowIso() };
    custom.push(row); localStorage.setItem(this.customKey, JSON.stringify(custom)); this.emit(); return row;
  }
...
class SupabaseStore implements DataStore {
  ...
  async setCompleted({ itemId, completed, actor, section, entryKey }: Parameters<DataStore["setCompleted"]>[0]) {
    const payload = { world_id: this.worldId, item_id: itemId, section, entry_key: entryKey, completed, completed_by: completed ? actor : null, completed_at: completed ? nowIso() : null, updated_at: nowIso(), deleted: false, deleted_by: null, deleted_at: null };
    const { data, error } = await this.client.from("mundinho_item_state").upsert(payload, { onConflict: "world_id,item_id" }).select().single(); if (error) throw error; return data as ItemState;
  }
  ...
  async importAll(payload: unknown, actor: Actor) { const normalized = normalizeBackup(payload, actor, this.worldId); const rows = Object.values(normalized.item_state); if (rows.length) { const { error } = await this.client.from("mundinho_item_state").upsert(rows, { onConflict: "world_id,item_id" }); if (error) throw error; } for (const item of normalized.custom_items) { const { id: _id, created_at: _created, updated_at: _updated, ...row } = item; const { error } = await this.client.from("mundinho_custom_items").insert(row); if (error) throw error; } }
  async migrateLegacy(actor: Actor) { const normalized = normalizeLegacy(actor, this.worldId); if (!normalized.found) return { found: false, migrated: 0 }; await this.importAll(normalized.payload, actor); localStorage.setItem("mundinho.legacyMigrationDone", "1"); return { found: true, migrated: normalized.count }; }
}
