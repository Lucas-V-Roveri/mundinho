...
type ContextValue = {
  actor: Actor; setActor: (actor: Actor) => void;
  soundEnabled: boolean; setSoundEnabled: (value: boolean) => void; playUiSound: (kind?: "click" | "success") => void;
  mode: DataMode; loading: boolean; content: ContentStore; states: Record<string, ItemState>; playerStates: Record<string, PlayerItemState>; customItems: CustomItem[]; legacyAvailable: boolean;
  ...
  deleteCustomItem: (id: string) => Promise<void>; exportData: () => Promise<void>; importData: (file: File) => Promise<void>; migrateLegacy: () => Promise<void>; refresh: () => Promise<void>;
};
...
export function AppProviders({ children }: { children: React.ReactNode }) {
  ...
  const refresh = React.useCallback(async () => {
    ...
    const [nextStates, nextPlayerStates, nextCustom] = await Promise.all([store.getStates(), store.getPlayerStates(), store.getCustomItems()]);
    setStates(nextStates); setPlayerStates(nextPlayerStates); setCustomItems(nextCustom);
  }, []);
  ...
  React.useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      let client: SupabaseClient | null = null; let worldId = DEFAULT_WORLD; let loadedContent = emptyContentStore;
      try {
        const config = await loadPublicRuntimeConfig(); worldId = config.worldId || DEFAULT_WORLD;
        client = getBrowserSupabase(config) as SupabaseClient;
        loadedContent = await loadContentStore(client); if (!cancelled) setContent(loadedContent);
      } catch (error) {
      ...
      const store = createDataStore(client, worldId); store.configureSubitems(buildSubitemMap(loadedContent.progression)); storeRef.current = store; setMode(store.mode);
      await store.ensureCompatibility(storedActor);
      await refresh(); unsubscribe = store.subscribe(() => void refresh()); if (!cancelled) setLoading(false);
    })().catch((error) => { console.error(error); toast({ title: "Falha ao abrir o mundinho", description: String(error), variant: "error" }); setLoading(false); });
    return () => { cancelled = true; unsubscribe(); window.cancelAnimationFrame(preferenceFrame); };
  }, [refresh, toast]);
  ...
  const playUiSound = React.useCallback((kind: "click" | "success" = "click") => {
    ...
    const AudioCtx = window.AudioContext || (window as typeof window & { webkitAudioContext?: typeof AudioContext }).webkitAudioContext; if (!AudioCtx) return;
    const context = audioRef.current ?? new AudioCtx(); audioRef.current = context;
    const oscillator = context.createOscillator(); const gain = context.createGain(); oscillator.type = "square"; oscillator.frequency.value = kind === "success" ? 880 : 440;
    gain.gain.setValueAtTime(0.035, context.currentTime); gain.gain.exponentialRampToValueAtTime(0.001, context.currentTime + 0.09); oscillator.connect(gain).connect(context.destination); oscillator.start(); oscillator.stop(context.currentTime + 0.09);
  }, [soundEnabled]);

  const toggleItem = React.useCallback(async (input: Parameters<ContextValue["toggleItem"]>[0]) => { if (!storeRef.current) return; await storeRef.current.setCompleted({ itemId: input.itemId, completed: input.completed, actor, section: input.section, entryKey: input.entryKey }); playUiSound(input.completed ? "success" : "click"); if (input.completed) toast({ title: "Advancement Made!", description: `${actor} marcou: ${input.label}`, variant: "advancement" }); await refresh(); }, [actor, playUiSound, refresh, toast]);
  ...
  const exportData = React.useCallback(async () => { if (!storeRef.current) return; const payload = await storeRef.current.exportAll(); const blob = new Blob([JSON.stringify(payload, null, 2)], { type: "application/json" }); const url = URL.createObjectURL(blob); const anchor = document.createElement("a"); anchor.href = url; anchor.download = `mundinho-backup-${new Date().toISOString().slice(0, 10)}.json`; anchor.click(); URL.revokeObjectURL(url); }, []);
  const importData = React.useCallback(async (file: File) => { if (!storeRef.current) return; const payload = JSON.parse(await file.text()); await storeRef.current.importAll(payload, actor); await refresh(); toast({ title: "Backup importado", description: "O progresso antigo ou novo foi mesclado sem apagar marcações existentes.", variant: "advancement" }); }, [actor, refresh, toast]);
  ...
  return <AppContext.Provider value={{ actor, setActor, soundEnabled, setSoundEnabled, playUiSound, mode, loading, content, states, playerStates, customItems, legacyAvailable, toggleItem, addCustomItem, deleteCustomItem, exportData, importData, migrateLegacy, refresh }}>{children}</AppContext.Provider>;
}
