...
type ContextValue = {
  ...
  mode: DataMode; loading: boolean; content: ContentStore; states: Record<string, ItemState>; customItems: CustomItem[]; legacyAvailable: boolean;
...
export function AppProviders({ children }: { children: React.ReactNode }) {
  ...
  const refresh = React.useCallback(async () => {
    ...
    setStates(nextStates); setCustomItems(nextCustom);
  }, []);
  ...
  React.useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      let client: SupabaseClient | null = null; let worldId = DEFAULT_WORLD;
      try {
        ...
        const loadedContent = await loadContentStore(client); if (!cancelled) setContent(loadedContent);
      } catch (error) {
      ...
      const store = createDataStore(client, worldId); storeRef.current = store; setMode(store.mode);
