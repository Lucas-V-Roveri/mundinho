      ...
      <Dialog open={open} onOpenChange={setOpen} title="Busca global" description="Mods, progressão, Extras, Amendments e Bastidores." className="w-[min(94vw,46rem)]">
        ...
        <div className="mt-4 max-h-[55vh] space-y-2 overflow-auto pr-1">
          {filtered.length ? filtered.map((row) => {
            ...
            return (
              <Link key={`${row.group}-${row.id}`} href={row.href as Route} onClick={() => { setOpen(false); setQuery(""); }} className={cn("pixel-card-interactive block border-4 border-night-950 bg-paper-50 p-3 text-ink-900", row.group === "Mods" && theme.frame)}>
                <span className="font-label text-xl text-blue-700">{row.group}</span>
                <strong className="mt-1 block text-sm">{row.title}</strong>
