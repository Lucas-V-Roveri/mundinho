  ...
  return (
    <div className="space-y-6 page-enter">
      <section className="grid gap-5 lg:grid-cols-[1.4fr_.8fr]">
        <Card surface="paper" className="relative overflow-hidden text-ink-900">
          ...
          <CardContent className="text-ink-900">
            ...
            <div className="mt-6 border border-night-950 bg-night-900 p-3 text-paper-50">
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              <div className={cn("border border-torch-700 bg-torch-100 p-4 text-ink-900", next && nextTheme.frame)}>
                ...
                {next ? <><strong className="mt-1 block">#{String(next.order).padStart(3, "0")} · {next.title}</strong><p className="mt-1 text-sm">{next.entry} · {next.phase} · risco {next.risk}</p><Link className="semantic-link mt-3 inline-block font-label text-xl" href={`/progressao#${next.id}` as Route}>ver na progressão</Link></> : <p className="mt-2">Tudo elegível já foi marcado. Aí sim dá para escolher por vontade.</p>}
              </div>
              <div className="border border-stone-500 bg-stone-100 p-4 text-ink-900">
                ...
                {last ? <><strong className="mt-1 block">{lastLabel}</strong><p className="mt-1 text-sm">{last.completed_by} · {new Intl.DateTimeFormat("pt-BR", { dateStyle: "short", timeStyle: "short" }).format(new Date(last.completed_at!))}</p></> : <p className="mt-2">Ainda sem marcações nesta base.</p>}
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4" aria-label="Resumo do Mundinho">
        <Stat icon="book" title="Guias" value={String(content.guides.length)} />
        <Stat icon="compass" title="Marcos" value={String(content.progression.length)} />
        <Stat icon="heart" title="Ideias extras" value={String(content.extras?.items.length ?? 0)} />
        <Stat icon="torch" title="Sincronização" value={mode === "supabase" ? "ONLINE" : "LOCAL"} />
      </section>

      <section className="grid gap-4 md:grid-cols-2">
        <Card surface="paper" className="text-ink-900">
          <CardHeader tone="wood"><CardTitle>gr1d + benamu</CardTitle></CardHeader>
          <CardContent className="grid gap-4 sm:grid-cols-2"><PlayerPanel actor="gr1d" src="/skins/gr1d.png" progression={content.progression} /><PlayerPanel actor="benamu" src="/skins/benamu.png" progression={content.progression} /></CardContent>
        </Card>
        <Card surface="paper" className="text-ink-900">
          ...
          <CardContent>
            ...
            <div className="trophy-shelf mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">{trophies.map((trophy) => {
              ...
              return (
                <Link
                  key={trophy.stateId}
                  href={`/progressao#${trophy.parent.id}` as Route}
                  title={done ? trophy.title : `derrote ${trophy.title} para desbloquear`}
                  className={cn(
                    "trophy-slot pixel-card-interactive bg-stone-100 p-2 text-center text-ink-900",
                    done ? "border-gold-700" : "border-stone-700",
                  )}
                >
                  <TrophyArt trophy={trophy} unlocked={done} />
                  <strong className="mt-2 block text-xs leading-4">{trophy.title}</strong>
                  <span className={cn("mt-1 block font-label text-base leading-4", done ? "text-gold-900" : "text-stone-700")}>{done ? `${markedBy} · ${markedAt ? new Intl.DateTimeFormat("pt-BR", { dateStyle: "short" }).format(new Date(markedAt)) : "feito"}` : "a descobrir"}</span>
                </Link>
              );
            })}</div>
          </CardContent>
...
function PlayerPanel({ actor, src, progression }: { actor: Actor; src: string; progression: ProgressionItem[] }) {
  ...
  return (
    <article className="border border-stone-500 bg-paper-50 p-3 text-center text-ink-900">
      <SkinFace src={src} name={actor} size="lg" className="mx-auto" />
      <strong className="mt-2 block font-label text-2xl">{actor}</strong>
      <p className="mt-1 text-sm">{completed}/{progression.length} marcos concluídos</p>
      <div className="mt-2 h-3 border border-night-950 bg-stone-300"><div className="h-full bg-grass-500" style={{ width: `${progression.length ? Math.round((completed / progression.length) * 100) : 0}%` }} /></div>
    </article>
  );
}
...
function TrophyArt({ trophy, unlocked }: { trophy: Trophy; unlocked: boolean }) {
  ...
  if (source) {
    return <ContentIcon src={source} alt={`Troféu: ${trophy.title}`} kind="boss" locked={!unlocked} className="mx-auto size-14" />;
  }
...
function Stat({ icon, title, value }: { icon: PixelIconName; title: string; value: string }) {
  return (
    <Card surface="stone" className="bg-stone-100 text-ink-900">
      <CardContent className="min-h-32">
        <span className="flex items-center gap-2 font-label text-xl text-ink-700"><PixelIcon name={icon} size={18} /> {title}</span>
        <strong className="mt-3 block break-words font-display text-xl leading-relaxed text-night-950 sm:text-2xl">{value}</strong>
      </CardContent>
    </Card>
  );
}
