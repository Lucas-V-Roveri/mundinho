...
export function ProgressionView() {
  ...
  React.useEffect(() => { setPhase(localStorage.getItem("mundinho.filter.phase") || "Todas"); setRisk(localStorage.getItem("mundinho.filter.risk") || "Todos"); }, []);
  ...
  return <div className="space-y-6 page-enter">
    ...
    <section className="grid gap-3 border-4 border-night-950 bg-stone-100 p-4 shadow-pixel sm:grid-cols-2">
      <label className="font-label text-xl">Fase<Select className="mt-1" value={phase} onChange={(event) => setPhaseSaved(event.target.value)}>{phases.map((value) => <option key={value}>{value}</option>)}</Select></label>
      <label className="font-label text-xl">Risco<Select className="mt-1" value={risk} onChange={(event) => setRiskSaved(event.target.value)}>{risks.map((value) => <option key={value}>{value}</option>)}</Select></label>
    </section>
    <div className="grid gap-4">
      {filtered.map((item) => <article key={item.id} id={item.id} className="scroll-mt-64">
        <Card surface={item.risk === "Severo" ? "night" : "paper"} interactive>
          ...
          <CardContent className="grid gap-4 lg:grid-cols-[1.1fr_.9fr]">
            <div className="space-y-3 text-sm leading-6"><Detail label="Equipamento mínimo" value={item.equipment} /><Detail label="Obrigatório" value={item.required} /><Detail label="Recomendação" value={item.soft} /><Detail label="Dependência" value={item.dependency} /><Detail label="Se despreparado" value={item.unprepared} /></div>
            <div><ChecklistItem itemId={item.id} label={item.title} phase={`${item.phase} · ${item.entry}`} section="progression" entryKey={item.entry} /><div className="mt-3 border-4 border-night-950 bg-stone-100 p-3 text-xs leading-5 text-ink-700"><strong>Complexidade:</strong> {item.complexity}<br/><strong>Reversibilidade:</strong> {item.reversibility}<br/><strong>Âncora vanilla:</strong> {item.vanilla}</div></div>
          </CardContent>
        </Card>
      </article>)}
    </div>
