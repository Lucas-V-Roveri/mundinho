...
function ItemChip({ item }: { item: RecipeIngredient }) {
  ...
  return <span title={tooltip} className="inline-flex items-center gap-2 border-2 border-night-950 bg-paper-50 px-2 py-1 text-xs text-ink-900 shadow-pixel-sm"><ContentIcon src={item.icon} alt={`Ícone de ${item.namePt}`} kind="item" className="size-7" /><span>{item.namePt}</span></span>;
}
...
export function RecipeDisplay({ craft }: { craft: Crafting }) {
  ...
  return <section className="border-4 border-night-950 bg-stone-100 p-4 shadow-pixel-sm">
    <div className="flex flex-wrap items-center justify-between gap-3"><ItemChip item={{ namePt: craft.title, nameEn: craft.nameEn || craft.title, icon: craft.icone || "/icons/item.svg" }} /><span className="font-label text-lg">confiança: {craft.confidence}</span></div>
